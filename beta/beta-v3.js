/** Volt Consumo — Beta v3.2 · runtime staged para reduzir contenção no login. */
...
let applicationModulesPromise = null;
...
function loadTestAccountModules() {
  const email = window.VOLT_BETA_API?.getSnapshot?.().account?.email?.trim().toLowerCase() || "";
  if (email !== "walflanribeiro@gmail.com") return null;
  return Promise.all([
