...
function scheduleRegionalCycles() {
...
function cycleLine(type, icon, name, cycle, context) {
  ...
  range.textContent = cycle?.current ? formatRange(cycle.current, context) : "No configurado";
...
function localizeCycleSettings() {
  ...
  if (heading) heading.textContent = "Ciclos de medición";
  const note = group.querySelector(":scope > .note");
  if (note) note.textContent = "Energía y agua tienen ciclos independientes. Cada servicio mantiene su propio período.";
  localizeCycleForm("energy", "Energía", "energía");
  localizeCycleForm("water", "Agua", "agua");
}

function localizeCycleForm(type, title, lower) {
  const form = document.querySelector(`#beta-${type}-cycle-form`);
  if (!form) return;
  const strong = form.querySelector("strong");
  if (strong) strong.textContent = title;
  const spans = form.querySelectorAll("label span");
  if (spans[0]) spans[0].textContent = "Día de inicio";
  if (spans[1]) spans[1].textContent = "Día de cierre";
  const button = form.querySelector("button[type=submit]");
  if (button) button.textContent = `Guardar ciclo de ${lower}`;
}
...
function localizeReadingHistory(context) {
  ...
  if (action) action.textContent = "Nueva lectura";
  ...
  document.querySelectorAll("#beta-reading-list .beta-reading-item").forEach((item) => {
    ...
    if (meta) {
      meta.textContent = meta.textContent
        ...
        .replace(/^Água · /, "Agua · ")
        .replace(/(\d{2})\/(\d{2})\/(\d{4}),?\s*(\d{2}):(\d{2})/, (_, d, m, y, h, min) => new Intl.DateTimeFormat(context.locale || "es-UY", {
          day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit"
        }).format(new Date(`${y}-${m}-${d}T${h}:${min}:00`)));
    }
    const summary = item.querySelector("summary");
    if (summary) summary.setAttribute("aria-label", "Acciones de la lectura");
    const buttons = item.querySelectorAll("button");
    if (buttons[0]) buttons[0].textContent = "Editar";
    if (buttons[1]) buttons[1].textContent = "Eliminar";
  });
}

function localizeReports() {
  setText("#beta-reports .eyebrow", "ANÁLISIS");
  setText("#beta-reports-title", "Informes");
  const pdf = document.querySelector("#beta-export-pdf");
  if (pdf) pdf.textContent = "Exportar PDF";
...
function formatRange(range, context) {
  const formatter = new Intl.DateTimeFormat(context.locale || "es-UY", { day: "2-digit", month: "short" });
  return `${formatter.format(new Date(range.start)).replace(".", "")} – ${formatter.format(new Date(range.end)).replace(".", "")}`;
}
