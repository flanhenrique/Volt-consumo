...
public struct VoltServiceSnapshot: Codable, Sendable, Equatable {
    ...
    public init(kind: VoltServiceKind, value: Double, unit: String, goal: Double? = nil, projectedValue: Double? = nil, estimatedCostBRL: Double? = nil, dailyAverage: Double? = nil, cycleElapsedDays: Int? = nil, cycleTotalDays: Int? = nil, lastReadingAt: Date? = nil) {
        ...
        self.estimatedCostBRL = estimatedCostBRL
        self.dailyAverage = dailyAverage
        self.cycleElapsedDays = cycleElapsedDays
        self.cycleTotalDays = cycleTotalDays
    ...
    public var cycleProgress: Double? {
        guard let elapsed = cycleElapsedDays, let total = cycleTotalDays, total > 0 else { return nil }
        return min(1, max(0, Double(elapsed) / Double(total)))
    }

    public var goalTone: VoltGoalTone {
        guard let progress = goalProgress else { return .unavailable }
...
public struct VoltWidgetSnapshot: Codable, Sendable, Equatable {
    ...
    public init(schemaVersion: Int = VoltWidgetSnapshot.currentSchemaVersion, generatedAt: Date = Date(), accountLabel: String? = nil, energy: VoltServiceSnapshot? = nil, water: VoltServiceSnapshot? = nil, tariffFlagLabel: String? = nil, totalEstimatedCostBRL: Double? = nil, accent: String? = "emerald", preferredTheme: String? = "system") {
        ...
        self.energy = energy
        self.water = water
        self.tariffFlagLabel = tariffFlagLabel
        self.totalEstimatedCostBRL = totalEstimatedCostBRL
        self.accent = accent
...
public enum VoltWidgetSnapshotError: Error, Equatable {
...
public enum VoltWidgetSnapshotCodec {
    ...
    public static func decode(_ data: Data) throws -> VoltWidgetSnapshot {
        let snapshot: VoltWidgetSnapshot
        do {
            snapshot = try decoder().decode(VoltWidgetSnapshot.self, from: data)
        } catch {
            throw VoltWidgetSnapshotError.invalidJSON
        }
        guard snapshot.isSupported else {
            throw VoltWidgetSnapshotError.unsupportedSchema(snapshot.schemaVersion)
        }
        return snapshot
    }

    public static func encode(_ snapshot: VoltWidgetSnapshot) throws -> Data {
...
public enum VoltWidgetFreshness: Equatable, Sendable {
    ...
    public static func evaluate(snapshotDate: Date, now: Date = Date()) -> Self {
        let age = max(0, now.timeIntervalSince(snapshotDate))
...
public struct VoltWidgetProjection: Equatable, Sendable {
    ...
    public init(service: VoltServiceSnapshot) {
        if let projected = service.projectedValue, projected >= 0 {
            value = projected
            isEstimated = true
            return
        }
        if let average = service.dailyAverage, let total = service.cycleTotalDays, total > 0 {
            value = max(service.value, average * Double(total))
            isEstimated = true
            return
        }
        value = service.value
...
public enum VoltWidgetFixtures {
    public static let demo = VoltWidgetSnapshot(
        generatedAt: Date(timeIntervalSince1970: 1_776_000_000),
        accountLabel: "Casa",
        energy: VoltServiceSnapshot(
            kind: .energy,
            value: 218,
            unit: "kWh",
            goal: 300,
            projectedValue: 286,
            estimatedCostBRL: 189.42,
            dailyAverage: 9.1,
            cycleElapsedDays: 24,
            cycleTotalDays: 31,
            lastReadingAt: Date(timeIntervalSince1970: 1_775_910_000)
        ),
        water: VoltServiceSnapshot(
            kind: .water,
            value: 8.4,
            unit: "m³",
            goal: 12,
            projectedValue: 10.7,
            estimatedCostBRL: 76.80,
            dailyAverage: 0.35,
            cycleElapsedDays: 24,
            cycleTotalDays: 31,
            lastReadingAt: Date(timeIntervalSince1970: 1_775_910_000)
        ),
        tariffFlagLabel: "Amarela",
