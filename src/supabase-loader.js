...
function sessionTimeout() {
  return new Promise((resolve) => {
    window.setTimeout(() => resolve({ data: { session: null }, error: null, timedOut: true }), SESSION_RECOVERY_TIMEOUT_MS);
  });
}

function hardenAuthClient(client) {
  ...
  client.auth.getSession = async () => {
    ...
    if (result?.timedOut) return { data: { session: null }, error: null };
    return result;
  };
  ...
  loadingPromise = new Promise((resolve, reject) => {
    ...
    script.addEventListener("load", () => {
      ...
      if (window.supabase?.createClient) {
        ...
        reject(new Error("O runtime Supabase foi carregado sem expor createClient."));
      }
    }, { once: true });
