...
const iosHandler = () => globalThis.webkit?.messageHandlers?.voltWidget;
const androidHandler = () => globalThis.voltAndroidWidget;
...
function applyNativeRoute(path) {
  ...
  if (applied) {
    postNative({ command: 'route-applied', path });
  }
...
export function syncVoltWidgets() {
  ...
  return postNative(buildWidgetSnapshot(input));
}
